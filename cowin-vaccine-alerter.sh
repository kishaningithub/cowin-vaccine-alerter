...
poll_cowin_and_send_alert() {
  ...
  cat dose-1-result.txt | xargs -I {} curl --retry 3 -fLG "https://api.telegram.org/${TELEGRAM_BOT_TOKEN}/sendMessage" --data-urlencode "chat_id=${TELEGRAM_BOT_CHAT_ID}" --data-urlencode "text={}"
}
